...
export function isValidCuit(cuit: string): boolean { // ! Quitar cuando corrijamos signin
...
export function isValidCuil(cuil: string): boolean { // ! Quitar cuando corrijamos signin
...
export function isValidDni(dni: string): boolean { // ! Quitar cuando corrijamos signin
    const dniNum = parseInt(dni, 10);
    return /^\d{7,8}$/.test(dni) && dniNum >= 1000000 && dniNum <= 99999999;
}

export function validarRazonSocial(nombre: string): boolean { // ! Este no lo estamos implementando -> revisar
...
export function validarPiso(piso: string): boolean { // ! Este no lo estamos implementando -> revisar
    return piso === '' || /^(\d+|PB)$/i.test(piso);
}

export function validarCiudad(ciudad: string): boolean {  // ! Este no lo estamos implementando -> revisar
    return ciudad.trim().length >= 2;
}

export function validarProvincia(provincia: string): boolean { // ! Este no lo estamos implementando -> revisar
    return provincias.some((p) => p.nombre === provincia);
}

export function validarProvinciaPorId(id: number): boolean { // ! Este no lo estamos implementando -> revisar
    return provincias.some((p) => p.id_provincia === id);
}

export function validarProvinciaPorISO(codigoIso: string): boolean { // ! Este no lo estamos implementando -> revisar
