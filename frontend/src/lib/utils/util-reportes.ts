...
export function haReportado(userId: number | undefined, proyectoId: number | undefined): boolean {
    ...
    const reportePendienteMock = mockReportes.some(r => 
 ...
 */
export async function crearReporte(datosReporte: Omit<Reporte, 'id_reporte' | 'created_at' | 'estado'>): Promise<Reporte> {
