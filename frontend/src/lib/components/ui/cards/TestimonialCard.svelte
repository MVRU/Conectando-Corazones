...
<div
	class={`testimonial-card flex h-[320px] min-h-[260px] w-full min-w-[260px] max-w-[380px] flex-col rounded-3xl border border-blue-200/30 bg-white px-7 py-8 shadow-[0_4px_28px_0_#bbeafe20] transition-all duration-500
...
>
	<!-- Estrellas dinámicas -->
	<div class="mb-4 flex gap-1 text-xl">
		{#each Array(5) as _, i}
			<svg
				class="{i < stars
					? 'text-yellow-400'
					: 'text-gray-300'} h-5 w-5 transition-transform duration-200 {i < stars
					? 'scale-100'
					: 'scale-90'}"
				fill="currentColor"
				viewBox="0 0 20 20"
