<script lang="ts">
	...
	function editarBien(id: string, campo: keyof Omit<Bien, 'id'>, valor: any) {
		bienes = bienes.map((b) => (b.id === id ? { ...b, [campo]: valor } : b));
		dispatch('change', { bienes });
	}
</script>
...
<div class="space-y-4">
	...
	<div class="border-2 border-dashed border-gray-300 rounded-lg p-4">
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-3">
			<div>
				<label for="nuevo-bien-nombre" class="block text-xs font-medium text-gray-600 mb-1"
					>Nombre *</label
				>
				<input
			...
			<div>
				<label for="nuevo-bien-cantidad" class="block text-xs font-medium text-gray-600 mb-1"
					>Cantidad *</label
				>
				<input
			...
			<div>
				<label for="nuevo-bien-unidad" class="block text-xs font-medium text-gray-600 mb-1"
					>Unidad *</label
				>
				<input
	...
	<p class="text-xs text-gray-500">
		Especifica los bienes materiales que necesitas: nombre, cantidad y unidad (ej: Libros, 50,
		unidades)
	</p>
</div>
