<script lang="ts">
  ...
  import { Users, Tag as TagIcon, CalendarDays, Hourglass } from 'lucide-svelte';
  ...
  function normalizarBeneficiarios(valor: unknown): number {
    ...
    if (typeof valor === 'number') return Number.isFinite(valor) ? Math.max(0, Math.floor(valor)) : 0;
...
<div class="grid gap-5 md:grid-cols-2">
  <!-- Personas alcanzadas -->
  <div class={claseCard} style="animation-delay:80ms" role="group" aria-labelledby="card-personas-titulo">
    <div class={claseHeader}>
      <span class={claseIcono} aria-hidden="true">
        <Users class="h-5 w-5 text-sky-600" aria-hidden="true" />
      </span>
  ...
  <!-- Categorías -->
  <div class={claseCard} style="animation-delay:140ms" role="group" aria-labelledby="card-categorias-titulo">
    <div class={claseHeader}>
      <span class={claseIcono} aria-hidden="true">
        <TagIcon class="h-5 w-5 text-sky-600" aria-hidden="true" />
      </span>
  ...
  <!-- Fecha de inicio -->
  <div class={claseCard} style="animation-delay:320ms" role="group" aria-labelledby="card-inicio-titulo">
    <div class={claseHeader}>
      <span class={claseIcono} aria-hidden="true">
        <CalendarDays class="h-5 w-5 text-sky-600" aria-hidden="true" />
      </span>
  ...
  <div class={claseCard} style="animation-delay:380ms" role="group" aria-labelledby="card-cierre-titulo">
    <div class={claseHeader}>
      <span class={claseIcono} aria-hidden="true">
        <Hourglass class="h-5 w-5 text-sky-600" aria-hidden="true" />
      </span>
