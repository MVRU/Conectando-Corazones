<script lang="ts">
	...
	let stage: 'sending' | 'waiting' | 'verified' | 'error' = 'sending';
	let code = '';
	let codeError = '';
	...
	function verificarCodigoManual() {
		if (code === '123456') {
			...
			codeError = 'El código ingresado no es válido';
		}
	}
</script>
...
{:else if stage === 'waiting'}
	...
	<div class="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
		...
		{#if codeError}
			<p class="mt-4 text-sm text-red-500">{codeError}</p>
		{/if}
...
{:else if stage === 'error'}
	...
	<div class="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
		...
		<div class="mt-8 flex flex-col gap-4 sm:flex-row">
			<Button label="Reintentar" on:click={() => (stage = 'sending')} />
