			...
			<div class="mt-2 flex flex-wrap gap-3 text-sm">
				...
				{:else}
					<span class="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-800">
						...
						{evidencias.length} {evidencias.length === 1 ? 'evidencia' : 'evidencias'} ({totalArchivos} {totalArchivos === 1 ? 'archivo' : 'archivos'})
					</span>
				{/if}
			</div>
		</div>
		...
		<div class="border-t border-gray-200 bg-white p-4">
			{#if evidencias.length === 0}
				<div class="text-center py-4">
					...
					<p class="mt-1 text-xs text-gray-500">Es necesario cargar al menos una evidencia para este objetivo</p>
				</div>
			{:else}
			...
			<div class="grid gap-6 md:grid-cols-2">
				...
				<div>
					...
					{#if evidenciasEntrada.length === 0}
						<p class="text-sm italic text-gray-400">Sin evidencias de entrada</p>
					{:else}
						<div class="space-y-3">
...
Usuario: {obtenerNombreUsuario(archivo.usuario_id || 0)}"
				...
				<div>
					...
					{#if evidenciasSalida.length === 0}
						<p class="text-sm italic text-gray-400">Sin evidencias de salida</p>
					{:else}
						<div class="space-y-3">
