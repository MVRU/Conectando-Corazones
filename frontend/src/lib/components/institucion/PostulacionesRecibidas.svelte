...
<script lang="ts">
	...
	import ProyectoSelector from '$lib/components/institucion/ProyectoSelector.svelte';
	import type {
		ProyectoConPostulaciones,
		Postulacion,
		ColaboradorActivo
	} from '$lib/types/Postulacion'; // FIX: usar Colaboracion con estado = 'pendiente'

	// Datos de ejemplo - en una implementación real vendrían del backend
	let proyectos: ProyectoConPostulaciones[] = [
		// FIX: no crear nuevos mocks, usar los que ya tenemos centralizados
		{
			id: 1,
			...
			postulaciones: [
				{
					...
					mensaje:
						'Me interesa mucho colaborar con este proyecto. Tengo experiencia en educación y puedo ayudar con la organización de los libros.'
				},
				{
					id: 2,
					...
					mensaje:
						'Nuestra fundación está interesada en colaborar con la donación de libros de texto y material didáctico.'
				},
				{
					id: 3,
					nombre: 'Juan Pérez',
					...
					mensaje:
						'Soy maestro jubilado y me gustaría ayudar con mi experiencia en el área educativa.'
				}
			],
			...
			postulaciones: [
				{
					...
					mensaje:
						'Nos interesa donar alimentos no perecederos y productos de primera necesidad semanalmente.'
				}
			],
			colaboradoresActivos: [
				{
	...
	setBreadcrumbs([BREADCRUMB_ROUTES.home, { label: 'Postulaciones recibidas' }]);
	...
	$: totalPostulaciones = proyectos.reduce(
		(acc, proyecto) => acc + proyecto.postulaciones.length,
		0
	);
	$: totalColaboradores = proyectos.reduce(
		(acc, proyecto) => acc + proyecto.colaboradoresActivos.length,
		0
	);
</script>
...
<main class="min-h-screen bg-gray-50">
	<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		<!-- Header -->
		<div class="mb-8">
			<h1 class="text-3xl font-bold text-[rgb(var(--base-color))]">Postulaciones recibidas</h1>
		...
		{#if proyectos.length > 1}
			<div class="mb-6">
				<ProyectoSelector {proyectos} {proyectoSeleccionado} onChange={cambiarProyecto} />
			</div>
		{/if}
			...
			<div class="mt-8 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
				...
				<div class="flex flex-wrap gap-3">
					{#if totalPostulaciones > 0}
						<button
							class="inline-flex items-center rounded-lg border border-blue-300 bg-blue-50 px-4 py-2 text-sm font-medium text-blue-700 transition-colors hover:bg-blue-100"
						>
					...
					{#if totalColaboradores > 0}
						<button
							class="inline-flex items-center rounded-lg border border-green-300 bg-green-50 px-4 py-2 text-sm font-medium text-green-700 transition-colors hover:bg-green-100"
						>
							📊 Generar reporte
						</button>
					{/if}
					...
					<button
						class="inline-flex items-center rounded-lg border border-purple-300 bg-purple-50 px-4 py-2 text-sm font-medium text-purple-700 transition-colors hover:bg-purple-100"
					>
