	...
	>
		...
		<div class="mb-6 grid gap-4 text-center sm:grid-cols-3">
			<div class="rounded-xl bg-blue-50 p-4">
				...
				<div class="mt-2 flex items-end justify-center gap-1 h-16">
					{#each tendenciasMensuales as mes, i}
						<div class="flex flex-col items-center">
							<div 
								class="w-4 bg-blue-500 rounded-t transition-all duration-1000"
			...
			<div class="rounded-xl bg-green-50 p-4">
				...
				<div class="mt-2 flex items-end justify-center gap-1 h-16">
					{#each tendenciasMensuales as mes, i}
						<div class="flex flex-col items-center">
							<div 
								class="w-4 bg-green-500 rounded-t transition-all duration-1000"
								style="height: {(mes.donaciones / 8000) * 40}px; transition-delay: {400 + i * 100}ms"
							></div>
							<span class="mt-1 text-xs text-gray-600">{mes.mes}</span>
						</div>
					{/each}
			...
			<div class="rounded-xl bg-orange-50 p-4">
				...
				<div class="mt-2 flex items-end justify-center gap-1 h-16">
					{#each tendenciasMensuales as mes, i}
						<div class="flex flex-col items-center">
							<div 
								class="w-4 bg-orange-500 rounded-t transition-all duration-1000"
								style="height: {(mes.beneficiarios / 100) * 40}px; transition-delay: {500 + i * 100}ms"
							></div>
							<span class="mt-1 text-xs text-gray-600">{mes.mes}</span>
						</div>
					{/each}
	...
	>
		...
		<div class="space-y-6">
			{#each proyectosDetallados as proyecto, i}
				<div 
					class="relative pl-8 pb-6 border-l-2 border-blue-200 last:border-l-0"
					class:opacity-0={!animate}
					style="transition-delay: {600 + i * 200}ms"
				>
					<!-- Punto del timeline -->
					<div class="absolute -left-2 top-0 h-4 w-4 rounded-full bg-blue-500 border-2 border-white shadow"></div>
					...
					<div class="bg-gray-50 rounded-xl p-4">
						...
						<!-- Mini historial -->
						<div class="mt-4">
							<h4 class="text-xs font-medium text-gray-700 mb-2">Progreso histórico:</h4>
							<div class="flex items-end gap-1 h-8">
								{#each proyecto.historial as punto, j}
									<div class="flex flex-col items-center">
										<div 
											class="w-2 bg-blue-400 rounded-t transition-all duration-1000"
											style="height: {(punto.valor / proyecto.objetivo) * 24}px; transition-delay: {800 + i * 200 + j * 50}ms"
										></div>
									</div>
								{/each}
							</div>
			...
			<div class="space-y-4">
				<div class="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
					...
					<div class="flex items-center gap-3">
						...
						<span class="text-lg font-bold text-blue-600">2024: {comparativaAnual[2024].proyectos}</span>
						<span class="text-xs text-green-600">
							+{Math.round(((comparativaAnual[2024].proyectos - comparativaAnual[2023].proyectos) / comparativaAnual[2023].proyectos) * 100)}%
						</span>
					</div>
				</div>
				...
				<div class="flex justify-between items-center p-3 bg-green-50 rounded-lg">
					...
					<div class="flex items-center gap-3">
						<span class="text-sm text-gray-600">2023: ${comparativaAnual[2023].donaciones.toLocaleString()}</span>
						<span class="text-lg font-bold text-green-600">2024: ${comparativaAnual[2024].donaciones.toLocaleString()}</span>
						<span class="text-xs text-green-600">
							+{Math.round(((comparativaAnual[2024].donaciones - comparativaAnual[2023].donaciones) / comparativaAnual[2023].donaciones) * 100)}%
						</span>
					</div>
				</div>
				...
				<div class="flex justify-between items-center p-3 bg-orange-50 rounded-lg">
					...
					<div class="flex items-center gap-3">
						...
						<span class="text-lg font-bold text-orange-600">2024: {comparativaAnual[2024].beneficiarios}</span>
						<span class="text-xs text-green-600">
							+{Math.round(((comparativaAnual[2024].beneficiarios - comparativaAnual[2023].beneficiarios) / comparativaAnual[2023].beneficiarios) * 100)}%
						</span>
					</div>
				</div>
			</div>
