...
<script lang="ts">
	...
	function getUrgenciaColor(urgencia: string) {
		switch (urgencia) {
			case 'alta': return 'border-red-200 bg-red-50 text-red-700';
			case 'media': return 'border-yellow-200 bg-yellow-50 text-yellow-700';
			case 'baja': return 'border-green-200 bg-green-50 text-green-700';
			...
			>
				...
				<div class="space-y-3">
					{#each proximasOportunidades as oportunidad, i}
						<div 
							class="rounded-lg border {getUrgenciaColor(oportunidad.urgencia)} p-4 transition-all duration-300 hover:shadow-md"
							class:opacity-0={!animate}
						...
						>
							...
							<p class="text-sm mb-2">{oportunidad.institucion}</p>
			...
			>
				...
				<div class="grid grid-cols-2 gap-3">
					{#each insigniasRecientes as insignia, i}
						...
						>
							<div class="text-2xl mb-1">{insignia.icono}</div>
					...
					{#each categoriasFavoritas as categoria, i}
						...
						>
							...
							<div class="w-full bg-gray-200 rounded-full h-2">
								<div 
									class="bg-gradient-to-r from-blue-500 to-blue-400 rounded-full h-2 transition-all duration-1000" 
