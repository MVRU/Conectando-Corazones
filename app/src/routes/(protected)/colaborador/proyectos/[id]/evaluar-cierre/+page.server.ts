...
export const actions: Actions = {
	aprobar: async ({ request, params, locals }) => {
		...
		try {
			...
			if (todosAprobaron) {
				...
				await proyectoRepo.updateEstado(proyectoId, 'completado');
				return {
					success: true,
	...
	rechazar: async ({ request, params, locals }) => {
		...
		try {
			...
			if (count >= 3) {
				await proyectoRepo.updateEstado(proyectoId, 'en_auditoria');
			} else {
				await proyectoRepo.updateEstado(proyectoId, 'pendiente_solicitud_cierre');
			}
