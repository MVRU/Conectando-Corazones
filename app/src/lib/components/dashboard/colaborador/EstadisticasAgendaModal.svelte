		...
		>
			...
			>
				<div class="flex items-center gap-3">
					...
					<div>
						<h2 class="text-lg font-bold text-white sm:text-xl">Agenda de voluntariado</h2>
						<p class="text-xs text-slate-400">Cronograma de actividades y fechas importantes</p>
					</div>
				</div>
				<button
					on:click={onClose}
					...
					{#if stats.proximosVencimientos.length > 0}
						...
						>
							<div class="mb-4 flex items-center justify-between">
								...
								<span class="text-xs font-medium text-rose-400">¡Próxima actividad!</span>
							</div>
							<div class="mb-1 truncate text-lg font-bold text-white sm:text-xl">
								{next.titulo}
							</div>
							<div class="text-xs text-slate-400">
								Fecha: {formatDate(next.fecha)} ({getRelativeTime(next.fecha)})
							</div>
							<div
							...
							>
								...
								<div class="flex items-center justify-between gap-4 sm:justify-end">
									<div class="text-right">
										...
										<div class="text-[10px] font-medium text-emerald-400">
