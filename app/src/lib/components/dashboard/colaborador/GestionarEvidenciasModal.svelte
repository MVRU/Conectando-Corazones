		...
		>
			...
			>
				<div class="flex items-center gap-3">
					...
					<div>
						<h2 class="text-lg font-bold text-white sm:text-xl">Mis evidencias</h2>
						<p class="text-xs text-slate-400">Subí comprobantes para tus colaboraciones activas</p>
					</div>
				</div>
				<button
					on:click={onClose}
				...
				<div class="grid gap-3">
					{#each proyectos as proyecto}
						...
						>
							...
							<div class="flex items-center gap-2">
								<a
									href="/colaborador/proyectos/{proyecto.id}/mis-aportes"
								...
								{#if puedeSubir}
									<a
										href="/colaborador/proyectos/{proyecto.id}/mis-aportes/nuevo"
					...
					{#if proyectos.length === 0}
						<div class="flex flex-col items-center justify-center py-12 text-center">
							...
							<h4 class="text-sm font-medium text-slate-300">Sin proyectos pendientes</h4>
							<p class="mt-1 text-xs text-slate-500">
								No tenés proyectos que requieran evidencias ahora.
							</p>
						</div>
					{/if}
				</div>
