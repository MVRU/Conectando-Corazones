<script lang="ts">
	...
	$: acciones = [
		...
		...(estadoVerificacion === 'pendiente' || estadoVerificacion === 'rechazada'
			? [
					{
						label:
...
<div class="relative mb-12">
					...
					>
						<div
							class={accion.disabled
								? 'text-slate-500'
								: accion.primary ||
									  (accion.secondary && (accion.color === 'rose' || accion.color === 'amber'))
									? 'text-white'
	...
	<div class="flex flex-col gap-3 sm:hidden">
		...
		{#each todasLasAcciones.filter((a) => a.primary || (a.secondary && (a.color === 'rose' || a.color === 'amber'))) as accion}
			<div class="relative w-full">
				{#if accion.href}
					<a
						href={accion.href}
