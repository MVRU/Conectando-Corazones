...
<div class="mb-3 flex flex-col {isOwn ? 'items-end' : 'items-start'}">
	<div
		class="group relative max-w-[85%] rounded-2xl px-3 py-2 shadow-sm transition-all hover:shadow-md md:max-w-[70%] md:px-4 md:py-2.5 {isOwn
	...
	>
		...
		<span
			class="mt-1.5 block text-right text-[10px] {isOwn
				? 'text-blue-100'
				: 'text-gray-400'} transition-opacity group-hover:opacity-100 {isOwn
				? 'opacity-90'
				: 'opacity-70'}"
		>
			{hora}
