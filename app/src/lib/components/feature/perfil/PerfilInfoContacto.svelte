		...
		[
			{
				...
				label: 'Usuario',
				valor: `@${perfilUsuario.username}`,
				mostrar: true
			},
			{
				icon: MapPin,
				theme: 'bg-blue-50 text-[#007FFF]',
				label: 'Ubicación',
				valor: perfilUsuario.localidad
			...
			},
			{
				...
				valor: perfilUsuario.created_at
					? new Date(perfilUsuario.created_at).toLocaleDateString('es-AR', {
							year: 'numeric',
							month: 'long',
							day: 'numeric'
						})
	...
	<div class="grid w-full grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-8">
				...
				{#each infoItems as item, i (item.label)}
					...
					>
						...
						<div class="min-w-0 flex-1">
							...
							<p
								class="mt-0.5 truncate text-sm font-medium text-gray-900 {item.capitalize
									? 'capitalize'
									: ''}"
							>
								{item.valor}
		...
		<div>
			...
			{#if puedeVerContactos}
				{#if contactosVisibles.length > 0}
					<div class="flex flex-col gap-2.5">
						{#each contactosVisibles as contacto, i (contacto.tipo_contacto + contacto.valor)}
							...
							>
								<div
									class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl {iconTheme} transition-transform duration-200 group-hover:scale-105"
								>
								...
								<div class="min-w-0 flex-1">
									<p class="text-xs font-semibold tracking-wide text-gray-400 uppercase">
										...
										{#if contacto.etiqueta}
											<span class="ml-1.5 font-normal text-gray-400 normal-case"
												>· {formatearEtiquetaContacto(contacto.etiqueta)}</span
											>
										{/if}
									</p>
									<p class="mt-0.5 truncate text-sm font-medium text-gray-900 select-all">
										{contacto.valor}
									</p>
								</div>
							</div>
				...
				{:else}
					<div
						class="rounded-2xl border border-dashed border-gray-200 bg-gray-50/80 p-6 text-center"
					>
			...
			{:else}
				<!-- Estado privado -->
				<div
					class="relative overflow-hidden rounded-2xl border border-gray-200 bg-linear-to-br from-gray-50 to-gray-100/60 p-6"
				>
					...
					<div class="relative z-10 flex flex-col items-center gap-3 text-center">
						<div
							class="flex h-12 w-12 items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-gray-200"
						>
							<Lock class="h-5 w-5 text-gray-400" />
						</div>
						<div>
							<h4 class="text-sm font-semibold text-gray-800">Información privada</h4>
							<p class="mt-1 text-xs leading-relaxed text-pretty text-gray-500">
								{mensajePrivacidad}
							</p>
						</div>
					</div>
