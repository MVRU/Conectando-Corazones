	...
	>
		...
		>
			...
			<p class="mb-4 text-sm text-gray-600">
				Seleccioná las formas en las que preferís participar en proyectos. Podés seleccionar o
				quitar opciones haciendo clic en ellas.
			</p>
				...
				<div class="grid gap-3">
					{#each TIPOS_PARTICIPACION_DESCRIPCION as tipo}
						...
						>
							<!-- Checkbox visual -->
							<div
								class="flex h-5 w-5 items-center justify-center rounded border-2 transition-colors {seleccionado
			...
			<div class="mb-6 rounded-lg {tiposMarcados.size === 0 ? 'bg-red-50' : 'bg-blue-50'} p-3">
				<p class="text-sm {tiposMarcados.size === 0 ? 'text-red-800' : 'text-blue-800'}">
					<span class="font-medium">{tiposMarcados.size}</span> tipo{tiposMarcados.size !== 1
						? 's'
						: ''} de participación seleccionado{tiposMarcados.size !== 1 ? 's' : ''}
