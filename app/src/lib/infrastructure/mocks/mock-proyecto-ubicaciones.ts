import type { ProyectoUbicacion } from '$lib/domain/types/ProyectoUbicacion';
import { getUbicacion } from '$lib/infrastructure/mocks/mock-ubicaciones';

const ubicacionesBase: ProyectoUbicacion[] = [
	{
		id_proyecto_ubicacion: 1,
		proyecto_id: 1,
		ubicacion_id: 1,
		ubicacion: getUbicacion(1)
	},
	{
		id_proyecto_ubicacion: 2,
		proyecto_id: 1,
		ubicacion_id: 2,
		ubicacion: getUbicacion(2)
	},
	{
		id_proyecto_ubicacion: 3,
		proyecto_id: 2,
		ubicacion_id: 3,
		ubicacion: getUbicacion(3)
	},
	{
		id_proyecto_ubicacion: 4,
		proyecto_id: 2,
		ubicacion_id: 4,
		ubicacion: getUbicacion(4)
	},
	{
		id_proyecto_ubicacion: 5,
		proyecto_id: 3,
		ubicacion_id: 5,
		ubicacion: getUbicacion(5)
	},
	{
		id_proyecto_ubicacion: 6,
		proyecto_id: 4,
		ubicacion_id: 6,
		ubicacion: getUbicacion(6)
	},
	{
		id_proyecto_ubicacion: 7,
		proyecto_id: 5,
		ubicacion_id: 7,
		ubicacion: getUbicacion(7)
	},
	{
		id_proyecto_ubicacion: 8,
		proyecto_id: 1,
		ubicacion_id: 7,
		ubicacion: getUbicacion(7)
	},
	{
		id_proyecto_ubicacion: 9,
		proyecto_id: 9,
		ubicacion_id: 8,
		ubicacion: getUbicacion(8)
	},
	{
		id_proyecto_ubicacion: 10,
		proyecto_id: 6,
		ubicacion_id: 4,
		ubicacion: getUbicacion(4)
	},
	{
		id_proyecto_ubicacion: 11,
		proyecto_id: 7,
		ubicacion_id: 2,
		ubicacion: getUbicacion(2)
	},
	{
		id_proyecto_ubicacion: 12,
		proyecto_id: 8,
		ubicacion_id: 6,
		ubicacion: getUbicacion(6)
	},
	{
		id_proyecto_ubicacion: 13,
		proyecto_id: 10,
		ubicacion_id: 8,
		ubicacion: getUbicacion(8)
	},
	{
		id_proyecto_ubicacion: 14,
		proyecto_id: 11,
		ubicacion_id: 9,
		ubicacion: getUbicacion(9)
	},
	{
		id_proyecto_ubicacion: 15,
		proyecto_id: 12,
		ubicacion_id: 10,
		ubicacion: getUbicacion(10)
	},
	{
		id_proyecto_ubicacion: 16,
		proyecto_id: 13,
		ubicacion_id: 11,
		ubicacion: getUbicacion(11)
	},
	{
		id_proyecto_ubicacion: 17,
		proyecto_id: 14,
		ubicacion_id: 12,
		ubicacion: getUbicacion(12)
	},
	{
		id_proyecto_ubicacion: 18,
		proyecto_id: 15,
		ubicacion_id: 13,
		ubicacion: getUbicacion(13)
	},
	{
		id_proyecto_ubicacion: 19,
		proyecto_id: 16,
		ubicacion_id: 14,
		ubicacion: getUbicacion(14)
	},
	{
		id_proyecto_ubicacion: 20,
		proyecto_id: 17,
		ubicacion_id: 15,
		ubicacion: getUbicacion(15)
	},
	{
		id_proyecto_ubicacion: 21,
		proyecto_id: 18,
		ubicacion_id: 16,
		ubicacion: getUbicacion(16)
	},
	{
		id_proyecto_ubicacion: 22,
		proyecto_id: 19,
		ubicacion_id: 17,
		ubicacion: getUbicacion(17)
	}
];

export const mockProyectoUbicaciones: ProyectoUbicacion[] = ubicacionesBase;
