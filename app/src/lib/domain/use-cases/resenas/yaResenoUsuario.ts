...
export class YaResenoUsuario {
	...
	async execute(autorId: number, tipoObjeto: TipoObjetoResena, idObjeto: number): Promise<boolean> {
		const resena = await this.resenaRepository.findByAutorAndObjeto(autorId, tipoObjeto, idObjeto);
		return resena !== null;
	}
}
